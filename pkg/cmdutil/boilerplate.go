...
func BoilerplateFlagsKafka(command *cobra.Command, serviceType string, envPrefix string) {
	...
	config.Int32Default(command, fmt.Sprintf("%s.kafka.max_open_requests", serviceKey), "kafka-max-open-requests", 1, "Enable TLS encryption", fmt.Sprintf("%s_KAFKA_MAX_OPEN_REQUESTS", envPrefix))

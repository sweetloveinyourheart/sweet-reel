...
const (
	VideoUploadedBucket = "video-uploaded"
	VideoUploadedFolder = "raw"
)
...
func (a *actions) PresignedUrl(ctx context.Context, request *connect.Request[proto.PresignedUrlRequest]) (*connect.Response[proto.PresignedUrlResponse], error) {
	...
	key := fmt.Sprintf("%s/%s/%s%s", VideoUploadedFolder, time.Now().Format("2006/01/02"), newVideo.GetID(), ext)
	url, err := a.s3Client.GenerateUploadPublicUri(key, VideoUploadedBucket, s3.UrlExpirationSeconds)
	if err != nil {
		logger.Global().Error("Failed to generate presigned URL",
			zap.String("key", key),
			zap.String("bucket", VideoUploadedBucket),
			zap.String("videoID", newVideo.GetID().String()),
			zap.Error(err))
		return nil, grpc.InternalError(errors.Wrapf(err, "failed to generate presigned URL for bucket %s, key %s", VideoUploadedBucket, key))
	}
